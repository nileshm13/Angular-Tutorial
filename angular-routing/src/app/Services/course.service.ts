...
export class CourseService {
    ...
    //we will call this method from authguard.service.ts for resolve route guard. timeout is to mock behavior of api calls taking some time to load, hence 4 sec specified here.
    getAllcourses() {
        return new Observable<Course[]>((obs) => {
            setTimeout(() => {
                obs.next(this.courses);
            }, 4000);
        });
    }
}